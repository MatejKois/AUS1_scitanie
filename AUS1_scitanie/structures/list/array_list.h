...
namespace structures
{
	...
	inline Structure& ArrayList<T>::assign(Structure& other)
	{
		if (this != &other)
		{
			...
			Array<T>* newArray = new Array<T>(*otherArrayList.array_);
			delete array_;
			array_ = newArray;
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		...
		{
			if (this->size_ != otherArrayList->size_)
			{
				return false;
			}
			for (int i = 0; i < this->size_; ++i)
			{
				if (this->at(i) != otherArrayList->at(i))
				{
					return false;
				}
			}
	...
	{
		Utils::rangeCheckExcept(index, this->size_, "Index out of range!");
	...
	inline T ArrayList<T>::removeAt(int index)
	{
		...
		{
			Array<T>::copy(*(this->array_), index + 1,
			               *(this->array_), index,
			               this->size_ - index - 1);
		}
		size_--;
