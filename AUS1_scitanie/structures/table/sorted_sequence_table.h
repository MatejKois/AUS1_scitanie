...
namespace structures
{
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(K key, int indexStart, int indexEnd, bool& found)
	{
		...
		K midKey = this->list_->at(middle)->getKey();
		...
		else
		{
			...
			{
				found = false;
				return key < midKey ? middle : middle + 1;
			}
			else
			{
				if (key < midKey)
				{
					return  this->indexOfKey(key, indexStart, middle, found);
				}
				else
				{
					return  this->indexOfKey(key, middle + 1, indexEnd, found);
				}
			}
		}
	}
